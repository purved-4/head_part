...
export class NotificationChatService {
  ...
  getThreadDetailsById(threadId: string): Observable<any> {
    return this.http
      .get(`${baseUrl}/api/chat/thread/${threadId}`, {
        withCredentials: true,
      })
      .pipe(map((res: any) => res.data));
  }
  ...
  SendNotificationAsRead(notificationId: string) {
    return this.http.put(`${baseUrl}/notifications/${notificationId}/read`, {});
  }

  markNotificationAsRead(recipientId: any) {
    return this.http.put(
      `${baseUrl}/notifications/read-all?recipientId=${recipientId}`,
      {},
    );
  }
...
getAllThreadCombinedPaginate(
    ...
    return this.http
      .get(
        `${baseUrl}/api/chat/findThreadCombined/paginated/${branchId}/${entityType}/${isResolved}/${type}?page=${page}&size=${size}&sort=updatedAt,desc`,
      )
      .pipe(
        map((res: any) => res.data),
        catchError((err) => throwError(err)),
