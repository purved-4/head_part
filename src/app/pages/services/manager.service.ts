...
export class ManagerService {
  ...
  getUsersByManagerId(userId: string): Observable<any[]> {
   ...
   getPortalByManagerId(managerId: string): Observable<any[]> {
    return this.http
      .get<any[]>(`${baseUrl}/manager/getAllManagersWithPortalsById/${managerId}`)
      .pipe(
        map((response: any) => response.data),
        catchError((error) => throwError(error))
      );
  ...
  return this.http.get<any>(`${baseUrl}/manager/owner/getAll`, { params }).pipe(
...
getManagerPortalPercentage(managerId: string): Observable<any> {
  return this.http.get<any>(`${baseUrl}/manager/getPercentages/${managerId}`).pipe(
    map(response => response.data), 
    catchError(error => throwError(() => error))
  );
}

getManagerWithChiefIdPaginated(chiefId: string, page = 0, size = 5): Observable<any> {
return this.http.get<any>(
  ...
  getManagerBalanceByPortal(portalId:any,managerId:any): Observable<any[]> {
    return this.http.get<any[]>(`${baseUrl}/manager/getManagerBalanceByPortal/${portalId}/${managerId}`);
  }
}
