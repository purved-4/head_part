...
export class ChiefService {
  ...
  getChiefsListByUserId(user: any,page?:any,pagesize?:any): Observable<Agent[]> {
    return this.http
  ...
  toggleChiefUserStatus(agentId: string, userId: any): Observable<Agent> {
   ...
   getAllPortalsByChiefId(agentId:any): Observable<any[]> {
    return this.http.get<any[]>(`${baseUrl}/chief/getPortalsByChiefId/${agentId}`).pipe(
      map((response: any) => response.data),
      catchError(error => throwError(error))
    );
  }
...
getChiefPortalPercentage(chiefId: string): Observable<any> {
  return this.http.get<any>(`${baseUrl}/chief/getPercentages/${chiefId}`).pipe(
...
addChiefDomain(chiefId: string,domain:any): Observable<any> {
  return this.http.post<any>(`${baseUrl}/chief/${chiefId}/domains?domain=${domain}`,{}).pipe(
    map(response => response.data), 
    catchError(error => throwError(() => error))
  );
}
...
updateChiefDomain(domainId: string,domain:any): Observable<any> {
  return this.http.put<any>(`${baseUrl}/chief/domains/${domainId}?domainName=${domain}`,{}).pipe(
    map(response => response.data), 
    catchError(error => throwError(() => error))
  );
}
...
getPortalsByChiefIDPaginated(chiefId: any, page?: number, size?: number) {
  ...
  getChiefBalanceByPortal(portalId:any,chiefId:any): Observable<any[]> {
    return this.http.get<any[]>(`${baseUrl}/chief/getChiefBalanceByPortal/${portalId}/${chiefId}`);
  }
