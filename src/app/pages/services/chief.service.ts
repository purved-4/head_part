...
getSelfRegistrationEligibility(chiefId: string): Observable<any> {
  ...
  createDynamicDistribution(payload: any) {
    return this.http
      .post<any>(`${baseUrl}/dynamic-distribution/create`, payload)
      .pipe(catchError((error) => throwError(() => error)));
  }

  updateDynamicDistribution(batchId: string, payload: any) {
    return this.http
      .put<any>(`${baseUrl}/dynamic-distribution/${batchId}`, payload)
      .pipe(catchError((error) => throwError(() => error)));
  }

  getDynamicDistribution(chiefId: string) {
    return this.http
      .get<any>(`${baseUrl}/dynamic-distribution/${chiefId}`)
      .pipe(catchError((error) => throwError(() => error)));
  }

  deactivateDynamicDistribution(batchId: string) {
    return this.http
