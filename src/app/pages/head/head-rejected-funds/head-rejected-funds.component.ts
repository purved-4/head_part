  ...
  export class HeadRejectedFundsComponent implements OnInit, OnDestroy {
    ...
    // Subjects for debounced search
    private upiSearchSubject = new Subject<string>();
    private bankSearchSubject = new Subject<string>();
    private payoutSearchSubject = new Subject<string>();
    ...
    private setupSearchDebounce(): void {
      // UPI search debounce
      this.upiSearchSubject
        .pipe(
          debounceTime(400),
          distinctUntilChanged(),
          switchMap((query) => {
            this.upiPage = 0; // reset to first page
        ...
        .subscribe((response) => this.handleUpiResponse(response));

      // Bank search debounce
      this.bankSearchSubject
        .pipe(
          debounceTime(400),
          distinctUntilChanged(),
          switchMap((query) => {
        ...
        .subscribe((response) => this.handleBankResponse(response));

      // Payout search debounce
      this.payoutSearchSubject
        .pipe(
          debounceTime(400),
          distinctUntilChanged(),
          switchMap((query) => {
        ...
        .subscribe((response) => this.handlePayoutResponse(response));
    }
    ...
    fetchUpiTopups(): void {
      ...
      this.fundService
        ...
        .subscribe((response: any) => this.handleUpiResponse(response));
    }

    private handleUpiResponse(response: any): void {
      let list: any[] = [];
      let total = 0;
      ...
      // sometimes backend returns paginated at top-level
        ...
        pageNum = typeof response.number === "number" ? response.number : pageNum;
        pageSize = typeof response.size === "number" ? response.size : pageSize;
      } else {
    ...
    fetchBankTopups(): void {
      ...
      this.fundService
        ...
        .subscribe((response: any) => this.handleBankResponse(response));
    }

    private handleBankResponse(response: any): void {
    ...
    fetchApprovedPayouts(): void {
      ...
      this.fundService
        ...
        .subscribe((response: any) => this.handlePayoutResponse(response));
    }

    private handlePayoutResponse(response: any): void {
      let list: any[] = [];
      let total = 0;
    ...
    // ============ Input Change Handlers (for debounce) ============
    onUpiSearchInputChange(query: string): void {
      this.upiSearchSubject.next(query);
    }

    onBankSearchInputChange(query: string): void {
      this.bankSearchSubject.next(query);
    }

    onPayoutSearchInputChange(query: string): void {
      this.payoutSearchSubject.next(query);
    }
    ...
    ngOnDestroy(): void {
      ...
      this.upiSearchSubject.complete();
      this.bankSearchSubject.complete();
      this.payoutSearchSubject.complete();
    }
