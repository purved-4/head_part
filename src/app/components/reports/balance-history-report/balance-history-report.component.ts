...
export class BalanceHistoryReportComponent implements OnInit {
  ...
  fetchSettleHistory(): void {
    ...
    const portalId = this.reportForm.get("portalId")?.value;
    const from = this.reportForm.get("fromDate")?.value;
    const to = this.reportForm.get("toDate")?.value;

    if (!entityId || !portalId) {
      this.errorMessage = "Select Entity and Portal to fetch settle history.";
      return;
    }
    if (!from || !to) {
  ...
  private mapSettleHistoryResponse(res: any): SettleHistoryItem[] {
    ...
    return data.map((item: any) => ({
      ...
      portalDomain:
        item.portalDomain ||
        item.domain ||
        this.portals.find((w) => w.id === item.portalId)?.name ||
        "Unknown",
        ...
        next: (res: any) => {
          if (Array.isArray(res)) {
            this.portals = res.map((item) => ({
              id: item.portalId,
              name: item.portalDomain || item.domain || item.id,
            }));
          } else if (res?.data) {
  ...
  exportToCSV(): void {
    ...
    const headers = [
      ...
      "Portal Balance Before",
      "Portal Balance After",
    ];
    const rows = this.settleHistory.map((item) => [
      ...
      item.portalBefore.toFixed(2),
      item.portalAfter.toFixed(2),
    ]);
  ...
  getFilteredHistory(): SettleHistoryItem[] {
    ...
    if (this.selectedPortal) {
      filtered = filtered.filter((item) =>
        item.portalDomain
          .toLowerCase()
          .includes(this.selectedPortal.toLowerCase()),
      );
    }
  ...
  getUniquePortals(): string[] {
    return [...new Set(this.settleHistory.map((item) => item.portalDomain))];
  }
  ...
  viewDetails(item: SettleHistoryItem): void {
    ...
    - Portal: ${item.portalBefore} → ${item.portalAfter}
  `;
    alert(modalContent);
  }
