...
export class PercentageLogService {
  ...
  private mockLogs: PercentageLog[] = [
    {
      id: "b3aba4ae",
      createdAt: "2025-12-02T16:04:29.151Z",
      portalId: "3930...e909",
      dw: 55,
      wp: 60,
      entityId: "sub-1",
      entityType: EntityType.SUB_ADMIN,
    },
    {
      id: "cc306685",
      createdAt: "2025-12-02T16:14:10.619Z",
      portalId: "3930...e909",
      dw: 34,
      wp: 34,
      entityId: "sub-2",
  ...
  getChildrenLogs(parentEntityId: string): Observable<PercentageLog[]> {
    const children = this.mockEntities.filter(
      (e) => e.parentId === parentEntityId,
    );
    const childIds = children.map((c) => c.id);
    const childLogs = this.mockLogs.filter((log) =>
      childIds.includes(log.entityId),
    );
