...
export class UtilsServiceService {
  ...
  getPortalByRoleIdAndRoleName(id: any, roleName: any): Observable<any[]> {
    switch (roleName.toString().toLowerCase()) {
      ...
      case "chief":
        return this.cnfService.getAllPortalsByChiefId(id).pipe(
          map((res) => res),
          catchError((err) => throwError(() => err)),
        );

      case "manager":
        return this.whoService.getPortalByManagerId(id).pipe(
          map((res) => res),
          catchError((err) => throwError(() => err)),
        );

      case "head":
        return this.headService.getAllHeadsWithPortalsById(id).pipe(
          map((res) => res),
          catchError((err) => throwError(() => err)),
        );

      case "branch":
        return this.userService.getPortalByBranchId(id).pipe(
          map((res) => res),
          catchError((err) => throwError(() => err)),
        );
  ...
  getOwnerBalanceByPortal(portalId: any): Observable<any[]> {
    return this.http.get<any[]>(
      `${baseUrl}/balance-history/getOwnerBalance/${portalId}`,
    );
  }
}
