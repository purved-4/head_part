...
this.updateMinDateTime();
  this.entityId =
    this.userStateService.getCurrentEntityId();

  this.currentRole =
    this.userStateService.getRole();
...
const selectedDate =
  new Date(this.effectiveFromNew).getTime();

const currentMinDate =
  new Date(this.minDateTime).getTime();

if (
  this.entityType !== "PORTAL" &&
  selectedDate < currentMinDate
) {
  ...
  if (this.entityType === "CHIEF") {
    ...
    submitObservable =
      this.chiefService.saveCurrencies(
        this.entityId,
        payload
      );
    ...
    submitObservable =
      this.comPartService.saveCurrencies(
        this.entityId,
        payload
      );
    ...
    submitObservable =
      this.portalService.saveCurrenciesByPortal(
        this.entityId,
        payload
      );
  ...
  submitObservable.subscribe({
    ...
    next: (res: any) => {
      ...
      this.existingData[key] = {
        ...
        effectiveFrom:
          responseData.effectiveFrom,

        currencyRates:
          responseData.currencyRates || [],
      };
  ...
  this.comPartService.deleteCurrencyRate(
    this.entityId,
    rateId,
    currencyId
  ).subscribe({
    ...
    next: (res: any) => {
      ...
      this.snackBar.show(
        res?.message || "Rate deleted successfully",
        true
      );
...
updateRate() {
  ...
  this.comPartService.updateCurrencyRate(
    this.entityId,
    this.editingRateId,
    ...
    next: (res: any) => {
      ...
      this.snackBar.show(
        res?.message || "Rate updated successfully",
        true
      );
...
isPastDate(date: string): boolean {
  ...
  const timeZone =
    this.tzService.getActiveTimeZone();
