...
export class WebhookDataComponent implements OnDestroy {
  ...
  // latest tp fields (object) without payload
  latestComPart: any = null;
  ...
  constructor(private poolingService: PoolingService) {
    ...
    poll$.subscribe((data: any) => {
      ...
      if (this.items.length > 0) {
        ...
        // keep tp fields (shallow copy) and remove payload for display
        const tp = { ...first };
        delete tp.payload;
        this.latestComPart = tp;

        // parse payload if it's a JSON string; tpwise keep raw
        try {
