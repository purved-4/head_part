...
@Component({
  selector: "app-add-spl",
  templateUrl: "./add-spl.component.html",
  styleUrl: "./add-spl.component.css",
})
export class AddSplComponent implements OnInit, OnDestroy {
  ...
  onSubmit(): void {
    ...
    this.crypto.addCrypto(formData).subscribe({
      next: (res) => {
        ...
        this.snackBar.show(res.message || "SPL added successfully", true);
        this.formSubmitted.emit();
      },
      error: (err) => {
        this.isSubmitting = false;
        this.snackBar.show(err?.error?.message || "Error adding SPL", false);
      },
    });
  }
