...
@Component({
  ...
  templateUrl: "./add-erc20.component.html",
  styleUrl: "./add-erc20.component.css",
})
export class AddErc20Component implements OnInit, OnDestroy {
  ...
  onSubmit(): void {
    ...
    this.crypto.addCrypto(formData).subscribe({
      next: (res) => {
        ...
        this.snackBar.show(res.message || "ERC-20 added successfully", true);
        this.formSubmitted.emit();
      },
      error: (err) => {
        this.isSubmitting = false;
        this.snackBar.show(err?.error?.message || "Error adding ERC-20", false);
      },
    });
  }
